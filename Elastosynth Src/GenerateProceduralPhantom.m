function [YM_phantoms, YM_phantoms_hetero, MetaData] = ProceduralPhantomGenerator(parameter_table,...
    ...
    %
    %   If parameter_table has a phantom_seed column (see
    %   GeneratePhantomParameterTable), the random generator is reset from
    %   that seed before every phantom, making each phantom reproducible.
    ...
    has_seed = ismember('phantom_seed', parameter_table.Properties.VariableNames);
    has_type = ismember('generation_type', parameter_table.Properties.VariableNames);
    ...
    for i = 1:height(parameter_table)
        ...
        if has_seed
            SetElastosynthSeed(phantom_params.phantom_seed, 1);
        end

        if has_type
            generation_type = string(phantom_params.generation_type);
        else
            generation_type = "Simple";
        end

        if generation_type == "LTI" || generation_type == "LTP"
        ...
        YM_phantoms_hetero(:,:,i) = YM_hetero;
        ...
        if has_seed
            metadata.phantom_seed = phantom_params.phantom_seed;
        else
            metadata.phantom_seed = NaN;
        end
